...
///    (`docs/ANE.md`). Two dies run only when two evaluations are in
...
public enum ANELinearBackend {
    ...
    /// `docs/ANE.md` for why the ceiling is where it is.
    ...
    final class Session {
        ...
        /// Two: the routed path holds at most two of one projection at once,
        /// and so does the CFG pipeline. A pool shallower than its deepest
        /// consumer deadlocks inside `take`, waiting on a slot the caller
        /// itself is holding — hence the bounded wait below.
